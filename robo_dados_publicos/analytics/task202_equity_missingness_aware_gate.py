...
def load_contract(path: str | Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    ...
    _stop(obj.get("base_main_sha") == "23e76bf07fa9dd3b1b6b4b22433139e231211245", "TASK202_BASE")
    cov = obj["coverage"]
    _stop(cov["active_school_denominator"] == 69, "TASK202_DENOM")
    _stop(cov["strong_links"] == 64, "TASK202_LINKS")
    _stop(cov["held"] == 5, "TASK202_HELD")
    _stop(abs(float(cov["rate"]) - 64 / 69) < 0.000001, "TASK202_RATE")
    _stop(float(cov["rate"]) >= float(cov["minimum_rate"]), "TASK202_RATE_MIN")
    _stop(float(cov["minimum_rate"]) == 0.92, "TASK202_RATE_MIN_PIN")
    _stop(cov["full_network"] is False, "TASK202_NOT_FULL")
    roster = obj["held_roster"]
    _stop(
        roster["source_evidence_path"]
        == "docs/evidence/TASK_199G_OFFICIAL_SPATIAL_TRIANGULATION_0.8.0.json",
        "TASK202_HELD_EVIDENCE_PATH",
    )
    _stop(roster["source_evidence_blob_sha"] == "3699e5e12c185c7b108f123dd20cab2584d1cc1f", "TASK202_HELD_EVIDENCE_SHA")
    _stop(len(roster["codes"]) == 5 and len(set(roster["codes"])) == 5, "TASK202_HELD_CODES")
    _stop(
        set(roster["codes"])
...
def build_task202_territory_profile(
    *,
    ...
    _stop(coverage["strong_links"] >= 64, "TASK202_MIN_LINKS")
    ...
    capabilities = [
        ...
        "SCHOOL_TO_SECTOR_LINK_COVERAGE_64_OF_69",
        "SCHOOL_TO_SECTOR_LINK_SUBSTANTIAL_COVERAGE_WITH_EXPLICIT_MISSINGNESS",
    ...
    _stop(product["row_count"] == 267, "TASK202_ROWS")
    return product
