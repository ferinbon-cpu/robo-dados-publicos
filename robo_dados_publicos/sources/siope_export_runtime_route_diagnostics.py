...
def summarize_post_click_requests(events: list[dict], config: dict, *, limit: int = DIAGNOSTIC_INVENTORY_LIMIT) -> dict:
    if limit < 1 or limit > DIAGNOSTIC_INVENTORY_LIMIT:
        raise SiopeRuntimeRouteProbeError("STOP_SIOPE_RUNTIME_ROUTE_DIAGNOSTICS_BAD_LIMIT")

    allowed_methods = set(config.get("candidate_methods") or ())
    allowed_resource_types = set(config.get("candidate_resource_types") or ())
    dedup: dict[tuple[str, str, str], dict] = {}
    excluded_non_http = 0
    excluded_method = 0
    excluded_resource_type = 0
    ...
    return {
        ...
        "excluded_resource_type_count": excluded_resource_type,
        "request_inventory": inventory,
    }


def diagnose_export_runtime_route(config: dict, *, runtime=None) -> dict:
    ...
    if any(raw.get(key) is not True for key in required_true):
        raise SiopeRuntimeRouteProbeError("STOP_SIOPE_RUNTIME_ROUTE_DIAGNOSTICS_RUNTIME_CONTRACT")
    if raw.get("candidate_route_network_sent") is not False:
        raise SiopeRuntimeRouteProbeError("STOP_SIOPE_RUNTIME_ROUTE_DIAGNOSTICS_NETWORK_SENT")

    summary = summarize_post_click_requests(list(raw.get("intercepted_requests") or []), config)
    diagnostics = {
    ...
    if summary["unique_route_shape_count"] < 1:
        raise SiopeRuntimeRouteProbeError(
            "STOP_SIOPE_RUNTIME_ROUTE_DIAGNOSTICS_NO_ELIGIBLE_REQUESTS",
            diagnostics=diagnostics,
        )

    return {
        "status": "PASS_M7_SIOPE_EXPORT_RUNTIME_ROUTE_DIAGNOSTICS_GATE",
        "gate_id": "M7_SIOPE_EXPORT_RUNTIME_ROUTE_DIAGNOSTICS_GATE_0_8_0",
        "software_version": config["software_version"],
        "diagnostic_status": "POST_CLICK_REQUEST_SHAPES_OBSERVED_NOT_SENT",
