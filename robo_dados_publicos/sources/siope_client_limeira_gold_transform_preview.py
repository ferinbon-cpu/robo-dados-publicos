...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW"
PERCENT_QUANTUM = Decimal("0.0001")
PER_CAPITA_QUANTUM = Decimal("0.01")


class GoldTransformPreviewError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise GoldTransformPreviewError(f"{ERROR}_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise GoldTransformPreviewError(f"{ERROR}_{code}")
...
def _decimal_field(data: dict, field: str, *, positive: bool = False) -> Decimal:
    ...
    if value is None or isinstance(value, bool):
        raise GoldTransformPreviewError(f"{ERROR}_{field}_VALUE")
    try:
        number = Decimal(str(value))
    except (InvalidOperation, ValueError, TypeError) as exc:
        raise GoldTransformPreviewError(f"{ERROR}_{field}_VALUE") from exc
    if not number.is_finite() or number < 0 or (positive and number <= 0):
        raise GoldTransformPreviewError(f"{ERROR}_{field}_RANGE")
    return number


def _pct(numerator: Decimal, denominator: Decimal) -> str:
    if denominator <= 0:
        raise GoldTransformPreviewError(f"{ERROR}_ZERO_DENOMINATOR")
...
def _per_capita(amount: Decimal, population: Decimal) -> str:
    if population <= 0:
        raise GoldTransformPreviewError(f"{ERROR}_ZERO_POPULATION")
...
def validate_config(config: dict) -> dict:
    expected = {
        ...
        "gate_id": "M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW_0_8_0",
        ...
        "mode": "OFFLINE_PINNED_VERIFIED_SILVER_ARITHMETIC_GOLD_PREVIEW",
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_REVIEW_0_8_0",
        "processing_authorized": False,
        "readback_review_gate": "M7_SIOPE_CLIENT_LIMEIRA_SILVER_DRIVE_READBACK_REVIEW_0_8_0",
        ...
        "silver_persistence_config_path": "config/source_expansion.siope_client_limeira_silver_drive_persistence.json",
...
def build_preview(config: dict, *, root: str | Path) -> tuple[dict, dict]:
    ...
    if not isinstance(data, dict) or len(data) != 52:
        raise GoldTransformPreviewError(f"{ERROR}_SILVER_DATA_SCHEMA")
    ...
    _require(data.get("NUM_ANO"), 2024, "YEAR")
