...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_FULL_SCHEMA_READONLY_VALIDATION"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_FULL_SCHEMA_READONLY_VALIDATION"


class SiopeClientLimeiraHistorical2023P6FullSchemaReadonlyValidationError(RuntimeError):
...
def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise SiopeClientLimeiraHistorical2023P6FullSchemaReadonlyValidationError(f"{ERROR}_{code}")
...
def _as_int(value, code: str) -> int:  # noqa: ANN001
    try:
        ...
        raise SiopeClientLimeiraHistorical2023P6FullSchemaReadonlyValidationError(
...
def validate_config(config: dict) -> dict:
    exact = {
        ...
        "mode": "ONE_REQUEST_GENERIC_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_FULL_52_FIELD_SCHEMA_VALIDATION",
        ...
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_FULL_SCHEMA_READONLY_VALIDATION_REVIEW_0_8_0",
    }
    for key, expected in exact.items():
        _require(config.get(key), expected, key.upper())
    _require(
        config.get("query"),
        {"ano": 2023, "periodo": 6, "uf": "SP", "municipality_code": 352690},
        "QUERY",
    )
    _require(
        config.get("policy"),
    ...
    return {
        "status": "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_FULL_SCHEMA_READONLY_VALIDATION_DESIGN",
...
def run_validation(config: dict, *, opener=None) -> dict:
    ...
    if (
        ...
        or _as_int(record.get("NUM_ANO"), "YEAR_INVALID") != 2023
