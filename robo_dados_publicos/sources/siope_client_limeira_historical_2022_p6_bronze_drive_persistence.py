...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_DRIVE_PERSISTENCE"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_DRIVE_PERSISTENCE"


class Historical2022P6DrivePersistenceError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise Historical2022P6DrivePersistenceError(f"{ERROR}_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise Historical2022P6DrivePersistenceError(f"{ERROR}_{code}")
...
def validate_config(config: dict, *, root: str | Path) -> dict:
    expected = {
        ...
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_DRIVE_PERSISTENCE_REVIEW_0_8_0",
        ...
        "review_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_SINGLE_RECORD_CAPTURE_REVIEW_0_8_0",
    ...
    bundle = {"bronze_bundle_contract": config["bundle_contract"], "manifest": manifest, "record": record}
...
def persist_bundle(config: dict, *, root: str | Path, drive=None) -> dict:
    ...
    if existing:
        raise Historical2022P6DrivePersistenceError(f"{ERROR}_REMOTE_NAME_COLLISION")
