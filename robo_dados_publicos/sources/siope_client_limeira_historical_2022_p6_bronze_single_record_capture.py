...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_SINGLE_RECORD_CAPTURE"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_SINGLE_RECORD_CAPTURE"


class Historical2022P6BronzeCaptureError(RuntimeError):
...
def _require(actual, expected, code: str, *, request_count: int = 0) -> None:
    if actual != expected:
        raise Historical2022P6BronzeCaptureError(f"{ERROR}_{code}", request_count=request_count)
...
def _as_int(value, code: str) -> int:  # noqa: ANN001
    try:
        ...
        raise Historical2022P6BronzeCaptureError(f"{ERROR}_{code}", request_count=1) from None


def validate_config(config: dict) -> dict:
    exact = {
        "gate_id": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_SINGLE_RECORD_CAPTURE_0_8_0",
        ...
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_SINGLE_RECORD_CAPTURE_REVIEW_0_8_0",
    }
    for key, expected in exact.items():
        _require(config.get(key), expected, key.upper())
    _require(config.get("query"), {"ano": 2022, "periodo": 6, "uf": "SP", "municipality_code": 352690}, "QUERY")
    _require(
        config.get("policy"),
...
def capture(config: dict, *, output_dir: str | Path, opener=None) -> dict:
    ...
    try:
        ...
        raise Historical2022P6BronzeCaptureError(f"{ERROR}_{str(exc)}", request_count=exc.request_count) from None
    ...
    _require(_as_int(record.get("NUM_ANO"), "NUM_ANO"), 2022, "YEAR", request_count=1)
