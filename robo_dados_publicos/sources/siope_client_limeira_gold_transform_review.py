...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_REVIEW"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_REVIEW"


class GoldTransformReviewError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise GoldTransformReviewError(f"{ERROR}_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise GoldTransformReviewError(f"{ERROR}_{code}")
...
def validate_config(config: dict) -> dict:
    expected = {
        ...
        "evidence_blob_sha": "04dfd1588817efea3e17720fe093138fc72e0671",
        "evidence_path": "docs/evidence/M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW_RUN_1_0.8.0.json",
        "expected_artifact_digest": "sha256:535ae1b476c6ddac62c30d315473516fa74b95fa879f0af28b911cf4c68b6fea",
        "expected_artifact_id": 9627788571,
        "expected_head_sha": "5eaa9401739a4965be93258d728e07879e576507",
        "expected_historical_tests": 109,
        "expected_job_id": 98361967339,
        "expected_metric_count": 8,
        "expected_run_id": 33024319543,
        "expected_unit_tests": 922,
        "gate_id": "M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_REVIEW_0_8_0",
        ...
        "mode": "OFFLINE_PINNED_GOLD_TRANSFORM_PREVIEW_REVIEW",
        "network_authorized": False,
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_GOLD_DRIVE_PERSISTENCE_DESIGN_0_8_0",
        "preview_config_path": "config/source_expansion.siope_client_limeira_gold_transform_preview.json",
        "preview_gate": "M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW_0_8_0",
...
def review(config: dict, *, root: str | Path) -> dict:
    ...
    _require(set(evidence), {"artifact_digest", "artifact_id", "artifact_name", "head_sha", "job_id", "qa", "result", "run_id", "status", "workflow"}, "EVIDENCE_KEYS")
    ...
    _require(evidence["status"], "PINNED_SUCCESSFUL_MANUAL_GOLD_TRANSFORM_PREVIEW_EVIDENCE", "EVIDENCE_STATUS")
    _require(evidence["workflow"], "M7 SIOPE CLIENT LIMEIRA GOLD TRANSFORM PREVIEW GATE", "WORKFLOW")
    _require(evidence["artifact_name"], f"siope-client-limeira-gold-transform-preview-{config['expected_run_id']}", "ARTIFACT_NAME")
    _require(
        evidence["qa"],
        {
    ...
    except (GoldTransformPreviewError, OSError, json.JSONDecodeError) as exc:
        raise GoldTransformReviewError(f"{ERROR}_LOCAL_REBUILD") from exc

    result = evidence.get("result")
    _require(result, local_result, "RESULT_DRIFT")
    _require(result.get("status"), "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW", "PREVIEW_STATUS")
