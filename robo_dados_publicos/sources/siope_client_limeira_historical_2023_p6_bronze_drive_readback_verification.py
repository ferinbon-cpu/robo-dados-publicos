...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_BRONZE_DRIVE_READBACK_VERIFICATION"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_BRONZE_DRIVE_READBACK_VERIFICATION"


class Historical2023P6DriveReadbackVerificationError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise Historical2023P6DriveReadbackVerificationError(f"{ERROR}_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise Historical2023P6DriveReadbackVerificationError(f"{ERROR}_{code}")
...
def validate_config(config: dict, *, root: str | Path) -> dict:
    expected = {
        ...
        "bundle_md5": "5aace303e15844abb01716534b57588f",
        ...
        "review_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_BRONZE_DRIVE_PERSISTENCE_REVIEW_0_8_0",
...
def verify_readback(config: dict, *, root: str | Path, drive=None) -> dict:
    ...
    if not isinstance(file_id, str) or not file_id:
        raise Historical2023P6DriveReadbackVerificationError(f"{ERROR}_REMOTE_ID_MISSING")

    with tempfile.TemporaryDirectory() as td:
        local = Path(td) / "historical_2023_p6_bronze_readback.json"
    ...
    try:
        ...
        raise Historical2023P6DriveReadbackVerificationError(f"{ERROR}_READBACK_JSON") from exc
