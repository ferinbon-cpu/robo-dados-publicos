...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_FULL_SCHEMA_READONLY_VALIDATION"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_FULL_SCHEMA_READONLY_VALIDATION"


class SiopeClientLimeiraFullSchemaReadonlyValidationError(RuntimeError):
...
def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise SiopeClientLimeiraFullSchemaReadonlyValidationError(f"{ERROR}_{code}")
...
def _as_int(value, code: str) -> int:  # noqa: ANN001
    try:
        ...
        raise SiopeClientLimeiraFullSchemaReadonlyValidationError(
...
def validate_config(config: dict) -> dict:
    exact = {
        "gate_id": "M7_SIOPE_CLIENT_LIMEIRA_FULL_SCHEMA_READONLY_VALIDATION_0_8_0",
        "source_id": "FNDE_SIOPE_DADOS_INFORMADOS_MUNICIPIOS_LIMEIRA",
        "software_version": "0.8.0",
        "mode": "ONE_REQUEST_GENERIC_SIOPE_CLIENT_LIMEIRA_FULL_52_FIELD_SCHEMA_VALIDATION",
        "manual_confirmation_required": True,
        "collection_authorized": False,
        "processing_authorized": False,
        "recurrence_authorized": False,
        "schedule_enabled": False,
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_FULL_SCHEMA_READONLY_VALIDATION_REVIEW_0_8_0",
    }
    for key, expected in exact.items():
        _require(config.get(key), expected, key.upper())
    _require(
        config.get("query"),
        {"ano": 2024, "periodo": 6, "uf": "SP", "municipality_code": 352690},
        "QUERY",
    )
    _require(
        config.get("policy"),
    ...
    return {
        "status": "PASS_M7_SIOPE_CLIENT_LIMEIRA_FULL_SCHEMA_READONLY_VALIDATION_DESIGN",
        ...
        "record_values_may_be_persisted": False,
        "response_body_may_be_persisted": False,
        "collection_authorized": False,
        "processing_authorized": False,
        "recurrence_authorized": False,
        "schedule_enabled": False,
...
def run_validation(config: dict, *, opener=None) -> dict:
    ...
    if (
        ...
        or _as_int(record.get("NUM_ANO"), "YEAR_INVALID") != 2024
    ...
    return {
        ...
        "recurrence_authorized": False,
        "schedule_enabled": False,
        "manual_single_full_schema_validation_authorization_consumed": True,
