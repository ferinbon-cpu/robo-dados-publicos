...
ERROR = "STOP_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS"


class SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(RuntimeError):
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_JSON_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_{code}")


def validate_config(config: dict, design_result: dict) -> None:
    exact = {
        "gate_id": "M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS_0_8_0",
        "source_id": "FNDE_SIOPE_DADOS_INFORMADOS_MUNICIPIOS_LIMEIRA",
        "software_version": "0.8.0",
        "release_status": "CANDIDATE",
        "mode": "PASSIVE_BOOLEAN_ONLY_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS",
        ...
        "next_gate": "M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS_REVIEW_0_8_0",
    }
    for key, expected in exact.items():
        _require(config.get(key), expected, f"CONFIG_{key.upper()}")

    _require(design_result.get("status"), "PASS_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS_DESIGN", "DESIGN_STATUS")
    ...
    _require(config.get("returned_boolean_fields"), [
    ...
    if "352690" in config["exact_application_url"] or "Limeira" in config["exact_application_url"]:
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_PILOT_VALUE")


class SystemChromeCdpDomSignatureRuntime:
    def _find_browser(self, config: dict) -> str:
        for name in config["browser_binary_candidates"]:
            path = shutil.which(name)
            if path:
                return path
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_BROWSER_UNAVAILABLE")
    ...
    def run_probe(self, config: dict) -> dict:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="siope-olinda-dom-signature-", ignore_cleanup_errors=True) as profile_text:
                ...
                if not surface or not all(surface.get(k) is True for k in ("scheme_matches", "host_matches", "path_matches", "query_empty", "ready_eligible")):
                    ...
                    raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(
                ...
                needles = [ids["service_document_declared_name"], ids["technical_callable_pattern_name"], *ids["technical_parameter_names"]]
                needle_literals = ",".join(json.dumps(value) for value in needles)
                signature_expr = f"""(() => {{
                  const needles = [{needle_literals}];
                  const root = document.documentElement;
                  const text = root ? (root.textContent || '') : '';
                  const hasInAttributes = (needle) => Array.from(document.querySelectorAll('*')).some((el) =>
                    Array.from(el.attributes || []).some((attr) => String(attr.value || '').includes(needle))
                  );
                  const present = needles.map((needle) => text.includes(needle) || hasInAttributes(needle));
                  return {{
                    service_document_declared_name_present: !!present[0],
                    technical_callable_pattern_name_present: !!present[1],
                    Ano_Consulta_present: !!present[2],
                    Num_Peri_present: !!present[3],
                    Sig_UF_present: !!present[4]
                  }};
                }})()"""
                evaluation = page_session.command("Runtime.evaluate", {"expression": signature_expr, "returnByValue": True})
                ...
                if set(signature) != expected_keys or any(type(signature[k]) is not bool for k in expected_keys):
                    raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_BOOLEAN_SIGNATURE_CONTRACT")
                ...
                if not all(final_surface.get(k) is True for k in ("scheme_matches", "host_matches", "path_matches", "query_empty", "ready_eligible")):
                    raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_SURFACE_DRIFT")

                shapes, candidates = summarize_blocked_requests(blocked, config)
                if len(shapes) > config["max_blocked_shapes"]:
                    raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_SHAPE_LIMIT")
                return {
...
def dry_run(config: dict, design_result: dict) -> dict:
    ...
    return {
        "status": "PASS_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS_DRY_RUN",
...
def run_dom_signature_diagnostics(config: dict, design_result: dict, *, runtime=None) -> dict:
    validate_config(config, design_result)
    runtime = runtime or SystemChromeCdpDomSignatureRuntime()
    ...
    signature = probe.get("boolean_signature") or {}
    expected_keys = set(config["returned_boolean_fields"])
    if set(signature) != expected_keys or any(type(signature[k]) is not bool for k in expected_keys):
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_BOOLEAN_SIGNATURE_CONTRACT")
    ...
    if any(shape.get("network_sent") is not False or shape.get("intercepted_before_network") is not True for shape in shapes):
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_BLOCK_POLICY")
    if any(candidate.get("candidate_dynamic_request") is not True for candidate in candidates):
        raise SiopeOfficialOlindaApiApplicationDomSignatureDiagnosticsError(f"{ERROR}_CANDIDATE_CLASSIFICATION")

    return {
        "status": "PASS_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_DOM_SIGNATURE_DIAGNOSTICS",
        "gate_id": config["gate_id"],
        "source_id": config["source_id"],
        "software_version": config["software_version"],
        "runtime_status": "BOOLEAN_ONLY_DOM_SIGNATURES_OBSERVED_WITH_UNAPPROVED_NETWORK_BLOCKED",
        "application_surface_verified": True,
        "fragment_present": bool(probe.get("fragment_present")),
        "fragment_value_returned": False,
        "boolean_signature": signature,
        "matched_signature_count": sum(1 for value in signature.values() if value),
