...
from robo_dados_publicos.sources.siope_client_limeira_historical_2023_p6_gold_transform_review import (
...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE"


class HistoricalGoldDrivePersistenceError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise HistoricalGoldDrivePersistenceError(f"{ERROR}_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise HistoricalGoldDrivePersistenceError(f"{ERROR}_{code}")
...
def _validated_gold_payload(config: dict, *, root: str | Path) -> dict:
    ...
    try:
        ...
        _require(review_result.get("status"), "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_TRANSFORM_REVIEW", "REVIEW_STATUS")
        ...
        _require(review_result.get("historical_collection_authorized"), False, "REVIEW_HISTORICAL_COLLECTION")
    ...
    except (HistoricalGoldTransformReviewError, HistoricalGoldTransformPreviewError, OSError, json.JSONDecodeError) as exc:
        raise HistoricalGoldDrivePersistenceError(f"{ERROR}_PREREQUISITE") from exc
    ...
    _require(gold_payload.get("identity", {}).get("municipality_code"), 352690, "MUNICIPALITY")
    _require(gold_payload.get("identity", {}).get("municipality_name"), "Limeira", "MUNICIPALITY_NAME")
    _require(gold_payload.get("identity", {}).get("state"), "SP", "STATE")
    _require(gold_payload.get("identity", {}).get("year"), 2023, "YEAR")
    _require(gold_payload.get("identity", {}).get("period"), 6, "PERIOD")
    ...
    _require(preview_result.get("historical_collection_authorized"), False, "PREVIEW_HISTORICAL_COLLECTION")
    return gold_payload
...
def validate_config(config: dict, *, root: str | Path) -> dict:
    expected = {
        ...
        "mode": "ONE_VALIDATED_HISTORICAL_GOLD_CREATE_ONLY",
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE_REVIEW_0_8_0",
        "overwrite_authorized": False,
        "preview_config_path": "config/source_expansion.siope_client_limeira_historical_2023_p6_gold_transform_preview.json",
        ...
        "review_config_path": "config/source_expansion.siope_client_limeira_historical_2023_p6_gold_transform_review.json",
        "review_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_TRANSFORM_REVIEW_0_8_0",
...
def persist(config: dict, *, root: str | Path, drive=None) -> dict:
    ...
    if existing:
        raise HistoricalGoldDrivePersistenceError(f"{ERROR}_REMOTE_NAME_COLLISION")

    with tempfile.TemporaryDirectory() as td:
        local = Path(td) / "historical_2023_p6_gold_payload.json"
