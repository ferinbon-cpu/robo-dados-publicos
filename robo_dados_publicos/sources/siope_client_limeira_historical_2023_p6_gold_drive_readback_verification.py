...
ERROR = "STOP_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_READBACK_VERIFICATION"
PASS = "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_READBACK_VERIFICATION"


class HistoricalGoldDriveReadbackVerificationError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise HistoricalGoldDriveReadbackVerificationError(f"{ERROR}_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise HistoricalGoldDriveReadbackVerificationError(f"{ERROR}_{code}")
...
def _expected_payload(config: dict, *, root: str | Path) -> tuple[dict, bytes]:
    ...
    try:
        ...
        raise HistoricalGoldDriveReadbackVerificationError(f"{ERROR}_EXPECTED_PAYLOAD_PREREQUISITE") from exc
...
def validate_config(config: dict, *, root: str | Path) -> dict:
    expected = {
        ...
        "mode": "ONE_EXISTING_DRIVE_HISTORICAL_GOLD_PAYLOAD_READBACK_VERIFY",
        "next_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_READBACK_REVIEW_0_8_0",
        "overwrite_authorized": False,
        "persistence_config_path": "config/source_expansion.siope_client_limeira_historical_2023_p6_gold_drive_persistence.json",
        ...
        "review_gate": "M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE_REVIEW_0_8_0",
...
def verify_readback(config: dict, *, root: str | Path, drive=None) -> dict:
    ...
    if not isinstance(file_id, str) or not file_id:
        raise HistoricalGoldDriveReadbackVerificationError(f"{ERROR}_REMOTE_ID_MISSING")

    with tempfile.TemporaryDirectory() as td:
        local = Path(td) / "historical_2023_p6_gold_readback.json"
    ...
    try:
        ...
        raise HistoricalGoldDriveReadbackVerificationError(f"{ERROR}_READBACK_JSON") from exc
