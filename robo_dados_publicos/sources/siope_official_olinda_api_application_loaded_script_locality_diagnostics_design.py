...
ERROR = "STOP_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_LOADED_SCRIPT_LOCALITY_DIAGNOSTICS_DESIGN"


class SiopeOfficialOlindaApiApplicationLoadedScriptLocalityDiagnosticsDesignError(RuntimeError):
    pass
...
def load_json(path: str | Path) -> dict:
    ...
    if not isinstance(payload, dict):
        raise SiopeOfficialOlindaApiApplicationLoadedScriptLocalityDiagnosticsDesignError(f"{ERROR}_JSON_OBJECT_REQUIRED")
    return payload


def _require(actual, expected, code: str) -> None:
    if actual != expected:
        raise SiopeOfficialOlindaApiApplicationLoadedScriptLocalityDiagnosticsDesignError(f"{ERROR}_{code}")


def run_design(config: dict, global_review: dict) -> dict:
    _require(config.get("gate_id"), "M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_LOADED_SCRIPT_LOCALITY_DIAGNOSTICS_DESIGN_0_8_0", "GATE")
    _require(config.get("software_version"), "0.8.0", "VERSION")
    _require(config.get("mode"), "OFFLINE_BOUNDED_EXPANDING_WINDOW_COUNT_DIAGNOSTICS_DESIGN", "MODE")
    ...
    _require(config.get("window_radii_chars"), [1024, 4096, 16384, 65536], "RADII")
    _require(len(config.get("returned_observations") or []), 33, "OBSERVATION_COUNT")
    _require(global_review.get("status"), "PASS_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_LOADED_SCRIPT_GLOBAL_RELATION_DIAGNOSTICS_REVIEW", "GLOBAL_REVIEW")
    _require(global_review.get("combined_contract_token_status"), "PROVEN_ONE_CALLABLE_SCRIPT_CONTAINS_ALL_PARAMETER_NAMES_ODATA_AND_FORMAT", "GLOBAL_COMBINED")
    _require(global_review.get("same_scope_or_expression_status"), "UNPROVEN_SAME_SCRIPT_COLOCATION_ONLY", "GLOBAL_SCOPE")
    _require(global_review.get("resource_route_contract_status"), "UNPROVEN", "GLOBAL_ROUTE")
    for key in (
    ...
    return {
        "status": "PASS_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_LOADED_SCRIPT_LOCALITY_DIAGNOSTICS_DESIGN",
        "gate_id": config["gate_id"],
        "source_id": config["source_id"],
        "software_version": config["software_version"],
        "design_mode": config["mode"],
        "observation_semantics": "TRANSIENT_ALREADY_LOADED_SCRIPT_EXPANDING_WINDOWS_INTEGER_COUNTS_ONLY",
        "window_radii_chars": config["window_radii_chars"],
