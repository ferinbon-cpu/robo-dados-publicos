...
def probe_chrome_image_only_pdf_accessibility(
    *,
    marker: str = "TASK109_IMAGE_ONLY_PDF_MARKER_48317",
) -> dict[str, Any]:
    """Probe whether local Chrome exposes text from a genuinely raster-only PDF."""
    chromedriver = shutil.which("chromedriver")
    chrome = shutil.which("google-chrome") or shutil.which("chromium")
    if not chromedriver or not chrome:
        return {
            "schema": "TASK109_CHROME_IMAGE_ONLY_PDF_PROBE_V1",
            "status": "UNAVAILABLE",
            "marker": marker,
            "pdf_text_empty": None,
            "marker_in_ax": False,
    ...
    try:
        ...
        raise LocalPdfCapabilityStop("TASK109_PYPDF_REQUIRED_FOR_SYNTHETIC_PROOF") from exc
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            raise LocalPdfCapabilityStop("TASK109_CHROMEDRIVER_NOT_READY")
        ...
        session_id = value.get("sessionId") or session.get("sessionId")
        if not session_id:
            raise LocalPdfCapabilityStop("TASK109_WEBDRIVER_NO_SESSION_ID")
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            source_html.write_text(
                (
                    ...
                    "color:black;padding:120px 40px;white-space:nowrap;'>"
                    + marker
                    + "</div></body></html>"
                ),
                encoding="utf-8",
            ...
            time.sleep(0.5)
            screenshot_response = _json_request(
                "GET",
                f"http://127.0.0.1:{port}/session/{session_id}/screenshot",
            )
            screenshot_b64 = screenshot_response.get("value")
            if not isinstance(screenshot_b64, str) or not screenshot_b64:
                raise LocalPdfCapabilityStop("TASK109_SYNTHETIC_SCREENSHOT_MISSING")
            png = root / "marker.png"
            png.write_bytes(base64.b64decode(screenshot_b64))
            ...
            _json_request(
                "POST",
                f"http://127.0.0.1:{port}/session/{session_id}/url",
                {"url": image_html.resolve().as_uri()},
            )
            time.sleep(0.5)
            printed = _json_request(
                "POST",
                f"http://127.0.0.1:{port}/session/{session_id}/goog/cdp/execute",
                {
                    "cmd": "Page.printToPDF",
            ...
            pdf_b64 = printed.get("value", {}).get("data")
            if not isinstance(pdf_b64, str) or not pdf_b64:
                raise LocalPdfCapabilityStop("TASK109_SYNTHETIC_IMAGE_PDF_MISSING")
            ...
            if extracted.strip():
                raise LocalPdfCapabilityStop("TASK109_SYNTHETIC_PDF_NOT_IMAGE_ONLY")
            ...
            return {
                ...
                "status": "PROBED",
                "marker": marker,
                "synthetic_png_bytes": png.stat().st_size,
                "synthetic_pdf_bytes": pdf.stat().st_size,
