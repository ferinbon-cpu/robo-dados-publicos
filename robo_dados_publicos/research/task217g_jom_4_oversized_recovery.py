...
DEFAULT_CONFIG = ROOT / "config/task217g_jom_4_oversized_recovery.v1.json"


class Task217GStop(RuntimeError):
    pass
...
def _stop(condition: bool, code: str) -> None:
    if not condition:
        raise Task217GStop(code)


def load_config(path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    cfg = json.loads(Path(path).read_text(encoding="utf-8"))
    _stop(cfg.get("schema") == "TASK217G_JOM_4_OVERSIZED_RECOVERY_V1", "TASK217G_SCHEMA")
    _stop(cfg.get("issue") == 680, "TASK217G_ISSUE")
    _stop(
        cfg.get("base_main_sha") == "2c45c0923e5903a295f1935e2518cd1c7f04b59b",
        "TASK217G_BASE",
    )
    targets = cfg["targets"]
    _stop(len(targets) == 4, "TASK217G_TARGET_COUNT")
    _stop(len({int(row["edition"]) for row in targets}) == 4, "TASK217G_TARGET_DUPLICATE")
    _stop(
        [int(row["edition"]) for row in targets] == [7243, 7253, 7270, 7287],
        "TASK217G_TARGET_IDENTITY",
    )
    _stop(all(str(row["url"]).startswith("https://ecrie.com.br/") for row in targets), "TASK217G_TARGET_HOST")
    net = cfg["network"]
    _stop(net["max_document_get_attempt_count"] == 4, "TASK217G_GET_COUNT")
    _stop(net["max_total_remote_get_count"] == 4, "TASK217G_REMOTE_COUNT")
    _stop(net["automatic_retry"] is False, "TASK217G_RETRY")
    _stop(net["rediscovery_authorized"] is False, "TASK217G_REDISCOVERY")
    _stop(
        int(net["max_bytes_per_document"]) * 4
        <= int(net["max_aggregate_document_bytes"]),
        "TASK217G_BUDGET_MATH",
    )
    _stop(cfg["authorization"]["live_authorized"] is False, "TASK217G_LIVE_DEFAULT")
    _stop(cfg["authorization"]["task217f_authorization_reuse_allowed"] is False, "TASK217G_217F_REUSE")
    _stop(cfg["authorization"]["task217d_authorization_reuse_allowed"] is False, "TASK217G_217D_REUSE")
    _stop(cfg["authorization"]["task018_authorization_reuse_allowed"] is False, "TASK217G_018_REUSE")
    _stop(cfg["promotion"]["runtime_may_promote"] is False, "TASK217G_PROMOTION")
    _stop(all(v is False for v in cfg["pre_authorization_remote_effects"].values()), "TASK217G_T0_EFFECT")
    return cfg
...
def _validate_download(
    ...
    *,
    ...
    _stop(meta.get("https") is True, "TASK217G_DOWNLOAD_HTTPS")
    _stop(meta.get("final_host") == cfg["network"]["allowed_document_host"], "TASK217G_DOWNLOAD_HOST")
    _stop(meta.get("content_type") == "application/pdf", "TASK217G_DOWNLOAD_CONTENT_TYPE")
    _stop(int(meta.get("remote_get_count") or 0) == 1, "TASK217G_DOWNLOAD_GET_COUNT")
    _stop(len(data) <= int(cfg["network"]["max_bytes_per_document"]), "TASK217G_DOCUMENT_BYTES")
    parsed = urlparse(str(item.get("url") or ""))
    _stop(parsed.scheme == "https", "TASK217G_DECLARED_URL_HTTPS")
    _stop((parsed.hostname or "").lower() == cfg["network"]["allowed_document_host"], "TASK217G_DECLARED_URL_HOST")
...
def execute_recovery(
    ...
    *,
    ...
    complete = len(results) == 4 and not failures
    return {
        "schema": "TASK217G_RECOVERY_RESULT_V1",
        "status": "PASS_COMPLETE_4_OVERSIZED_DOCUMENT_RECOVERY" if complete else "PARTIAL_4_OVERSIZED_DOCUMENT_RECOVERY_NO_ABSENCE_INFERENCE",
...
def validate_offline_carrier(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    cfg = load_config(config_path)
    return {
        "schema": "TASK217G_OFFLINE_CARRIER_VALIDATION_V1",
        "status": "PASS",
