#!/usr/bin/env python3
"""Manual read-only discovery of the public SIOPE export control contract."""
...
def run_gate(config_path: str | Path, *, dry_run: bool = False) -> tuple[dict, int]:
    try:
        if not _identity_ok():
            raise SiopeExportContractDiscoveryError("STOP_SIOPE_EXPORT_CONTRACT_RELEASE_IDENTITY")
        path = Path(config_path)
        if not path.is_absolute():
            path = ROOT / path
        config = load_export_contract_discovery_config(path)
        if dry_run:
            return {
                "status": "PASS_M7_SIOPE_EXPORT_CONTRACT_DISCOVERY_DRY_RUN",
        ...
        return discover_export_contract(config), 0
    except SiopeExportContractDiscoveryError as exc:
        return {
            "status": "STOP_M7_SIOPE_EXPORT_CONTRACT_DISCOVERY_GATE",
            "reason": str(exc),
        ...
        return {
            "status": "STOP_M7_SIOPE_EXPORT_CONTRACT_DISCOVERY_UNEXPECTED",
            **_safety_fields(),
        }, 20
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="config/source_expansion.siope_export_contract_discovery_gate.json",
    )
