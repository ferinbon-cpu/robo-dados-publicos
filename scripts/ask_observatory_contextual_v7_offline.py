...
from robo_dados_publicos.productization.school_infrastructure_context_execution import (  # noqa: E402
    Task218InfrastructureStop,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Executa a camada contextual V7 offline, incluindo INFRA_Q2 com identidade escolar JOM comprovada."
    )
    parser.add_argument("--ask", help="Pergunta em português.")
    parser.add_argument("--task218-status", action="store_true", help="Imprime validação e blockers contextuais remanescentes.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.task218_status:
        if args.ask:
            print("STOP_TASK218:ASK_AND_STATUS_ARE_MUTUALLY_EXCLUSIVE", file=sys.stderr)
            return 2
    ...
    if not args.ask:
        print("STOP_TASK218:ASK_REQUIRED", file=sys.stderr)
        return 2

    generated_at, software_version = _runtime_values()
    try:
        answer = execute_contextual_query_v7(
            args.ask,
        ...
        if args.json:
            ...
            sys.stdout.write(render_contextual_answer_v7(answer)["markdown"])
    except Task218InfrastructureStop as exc:
        print(f"STOP_TASK218:{exc}", file=sys.stderr)
        return 2
