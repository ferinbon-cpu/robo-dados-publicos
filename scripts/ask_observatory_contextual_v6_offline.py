...
from robo_dados_publicos.productization.proven_cross_product_execution import (  # noqa: E402
    Task215CrosswalkStop,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Executa a camada contextual V6 offline, incluindo o crosswalk "
            "estruturado e escopado LOA 2026 -> TCE Jan-Jul/2026 da TASK215."
        )
    )
    ...
    parser.add_argument(
        "--task215-status",
        action="store_true",
        help="Imprime contrato/validação e os blockers remanescentes.",
    )
    parser.add_argument(
        "--reference-date",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.task215_status:
        if args.ask:
            print("STOP_TASK215:ASK_AND_STATUS_ARE_MUTUALLY_EXCLUSIVE", file=sys.stderr)
            return 2
        ...
        payload = {
            ...
            "join_strength": contract["join_strength"],
        }
        sys.stdout.write(
    ...
    if not args.ask:
        print("STOP_TASK215:ASK_REQUIRED", file=sys.stderr)
        return 2

    generated_at, software_version = _runtime_values()
    try:
        answer = execute_contextual_query_v6(
            args.ask,
        ...
        if args.json:
            ...
            sys.stdout.write(render_contextual_answer_v6(answer)["markdown"])
    except Task215CrosswalkStop as exc:
        print(f"STOP_TASK215:{exc}", file=sys.stderr)
        return 2
