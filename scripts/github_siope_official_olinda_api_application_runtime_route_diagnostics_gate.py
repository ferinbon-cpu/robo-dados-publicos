...
from robo_dados_publicos.sources.siope_official_olinda_api_application_runtime_route_diagnostics import (  # noqa: E402
...
def run_gate(*, dry: bool = False) -> dict:
    ...
    design = load_json(ROOT / config["design_config_path"])
    return dry_run(config, design) if dry else run_application_route_diagnostics(config, design)
...
def main() -> int:
    ...
    try:
        ...
        exit_code = 0
    except SiopeOfficialOlindaApiApplicationRuntimeRouteDiagnosticsError as exc:
        result = {
            "status": "STOP_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_RUNTIME_ROUTE_DIAGNOSTICS",
            "error_code": str(exc),
            **exc.diagnostics,
