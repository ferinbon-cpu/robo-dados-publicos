#!/usr/bin/env python3
"""Manual read-only SIOPE export call-site route discovery gate."""
...
def run_gate(config_path: str | Path, *, dry_run: bool = False) -> tuple[dict, int]:
    try:
        if not _identity_ok():
            raise SiopeExportCallsiteRouteError("STOP_SIOPE_EXPORT_CALLSITE_RELEASE_IDENTITY")
        path = Path(config_path)
        if not path.is_absolute():
            path = ROOT / path
        config = load_export_callsite_route_config(path)
        if dry_run:
            return {
                "status": "PASS_M7_SIOPE_EXPORT_CALLSITE_ROUTE_DISCOVERY_DRY_RUN",
        ...
        return discover_export_callsite_routes(config), 0
    except SiopeExportCallsiteRouteError as exc:
        return {
            "status": "STOP_M7_SIOPE_EXPORT_CALLSITE_ROUTE_DISCOVERY_GATE",
            "reason": str(exc),
        ...
        return {
            "status": "STOP_M7_SIOPE_EXPORT_CALLSITE_ROUTE_DISCOVERY_UNEXPECTED",
            **_safety_fields(),
        }, 21
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="config/source_expansion.siope_export_callsite_route_gate.json",
    )
