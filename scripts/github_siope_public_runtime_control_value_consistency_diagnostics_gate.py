...
from robo_dados_publicos.sources.siope_public_runtime_control_value_consistency_diagnostics import (  # noqa: E402
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        _emit(_closed("PASS_M7_SIOPE_PUBLIC_RUNTIME_CONTROL_VALUE_CONSISTENCY_DIAGNOSTICS_DRY_RUN"), args.output)
        return 0

    try:
        result = diagnose_control_value_consistency(config, public_config, design_config)
    except SiopePublicRuntimeControlValueConsistencyDiagnosticsError as exc:
        _emit(
            _closed(
                "STOP_M7_SIOPE_PUBLIC_RUNTIME_CONTROL_VALUE_CONSISTENCY_DIAGNOSTICS",
                reason=str(exc),
        ...
        _emit(
            _closed(
                "STOP_M7_SIOPE_PUBLIC_RUNTIME_CONTROL_VALUE_CONSISTENCY_DIAGNOSTICS",
                reason="STOP_SIOPE_PUBLIC_RUNTIME_CONTROL_VALUE_CONSISTENCY_DIAGNOSTICS_UNEXPECTED_RUNTIME_ERROR",
            ),
            args.output,
        )
        return 37
