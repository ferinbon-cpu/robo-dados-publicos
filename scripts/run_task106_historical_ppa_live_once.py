...
RESULT = RUNTIME / "task106_result.json"
...
def _task106_payload(payload: dict) -> dict:
    converted = dict(payload)
    converted["schema"] = "TASK_106_HISTORICAL_PPA_LIVE_RESULT_V1"
    converted["task"] = "TASK_106_SINGLE_USE_HISTORICAL_PPA_PRIMARY_EVIDENCE_PYPDF"
    status = str(converted.get("overall_status") or "")
    converted["overall_status"] = status.replace("TASK104", "TASK106")
    converted["parser"] = {
        "name": "pypdf",
...
def main() -> int:
    ...
    try:
        ...
        payload = _task106_payload(payload)
    except (HistoricalPpaLiveStop, LocalPdfCapabilityStop) as exc:
        payload = {
            "schema": "TASK_106_HISTORICAL_PPA_LIVE_RESULT_V1",
            "task": "TASK_106_SINGLE_USE_HISTORICAL_PPA_PRIMARY_EVIDENCE_PYPDF",
            "overall_status": "STOP_TASK106_FATAL_CONTRACT_BUDGET_OR_PARSER_ERROR",
            "error": str(exc),
    ...
    payload["owner_authorization_path"] = (
        "docs/evidence/TASK_106_OWNER_AUTHORIZATION_PRE_RUN_0.8.0.json"
    )
