...
CONFIG = ROOT / "config/source_expansion.siope_official_olinda_api_application_fragment_target_structure_diagnostics.json"
...
def run_gate(*, dry: bool = False) -> dict:
    ...
    return dry_run(config, design) if dry else run_fragment_target_structure_diagnostics(config, design)
...
def main() -> int:
    ...
    try:
        ...
        exit_code = 0
    except SiopeOfficialOlindaApiApplicationFragmentTargetStructureDiagnosticsError as exc:
        result = {
            "status": "STOP_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_FRAGMENT_TARGET_STRUCTURE_DIAGNOSTICS",
            "error_code": str(exc),
            **exc.diagnostics,
