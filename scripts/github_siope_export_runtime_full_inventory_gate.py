...
from robo_dados_publicos.sources.siope_export_runtime_full_inventory import diagnose_full_runtime_inventory
...
def run_gate(config_path: str | Path, *, dry_run: bool = False, runtime=None) -> tuple[dict, int]:
    try:
        if not _identity_ok():
            raise SiopeRuntimeRouteProbeError("STOP_SIOPE_RUNTIME_FULL_INVENTORY_RELEASE_IDENTITY")
        ...
        if dry_run:
            return {
                "status": "PASS_M7_SIOPE_EXPORT_RUNTIME_FULL_INVENTORY_DRY_RUN",
        ...
        return diagnose_full_runtime_inventory(config, runtime=runtime), 0
    except SiopeRuntimeRouteProbeError as exc:
        return {
            "status": "STOP_M7_SIOPE_EXPORT_RUNTIME_FULL_INVENTORY_GATE",
            "reason": str(exc),
        ...
        return {
            "status": "STOP_M7_SIOPE_EXPORT_RUNTIME_FULL_INVENTORY_UNEXPECTED",
            **_safety_fields(),
        }, 25
