...
CONFIG = ROOT / "config/source_expansion.siope_official_olinda_api_application_loaded_script_signature_diagnostics.json"
...
def run_gate(*, dry: bool = False) -> dict:
    ...
    return dry_run(config, design) if dry else run_loaded_script_signature_diagnostics(config, design)
...
def main() -> int:
    ...
    try:
        ...
        exit_code = 0
    except SiopeOfficialOlindaApiApplicationLoadedScriptSignatureDiagnosticsError as exc:
        result = {
            "status": "STOP_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_LOADED_SCRIPT_SIGNATURE_DIAGNOSTICS",
            "error_code": str(exc),
            **exc.diagnostics,
