...
def run_gate(config_path: str | Path, *, dry_run: bool = False, runtime=None) -> tuple[dict, int]:
    try:
        if not _identity_ok():
            raise SiopeRuntimeRouteProbeError("STOP_SIOPE_RUNTIME_ROUTE_PROBE_RELEASE_IDENTITY")
        ...
        if dry_run:
            return {
                "status": "PASS_M7_SIOPE_EXPORT_RUNTIME_ROUTE_PROBE_DRY_RUN",
        ...
        return probe_export_runtime_route(config, runtime=runtime), 0
    except SiopeRuntimeRouteProbeError as exc:
        return {
            "status": "STOP_M7_SIOPE_EXPORT_RUNTIME_ROUTE_PROBE_GATE",
            "reason": str(exc),
        ...
        return {
            "status": "STOP_M7_SIOPE_EXPORT_RUNTIME_ROUTE_PROBE_UNEXPECTED",
            **_safety_fields(),
        }, 24
