#!/usr/bin/env python3
"""Run the TASK 009 metadata package route probe only after exact one-shot authorization."""
...
def main() -> int:
    ...
    if args.mode == "dry-run":
        _emit({
            "status": "PASS_TASK009_ROUTE_PROBE_DRY_RUN_NO_NETWORK",
            "source_get_count": 0,
    ...
    if not ACTUAL_AUTH.exists():
        _emit({"status": "STOP_METADATA_PACKAGE_ROUTE_PROBE_NOT_AUTHORIZED", "source_get_count": 0})
        return 13
    if not args.authorization_id or not args.workflow_run_number or not args.workflow_run_attempt or not args.workflow_ref:
        _emit({"status": "STOP_METADATA_PACKAGE_ROUTE_PROBE_RUN_IDENTITY_REQUIRED", "source_get_count": 0})
        return 13
    ...
    try:
        ...
        observation = MetadataPackageRouteProbe().run()
    except MetadataPackageProbeError as exc:
        _emit({
            "status": "STOP_METADATA_PACKAGE_ROUTE_PROBE",
            "reason": str(exc),
        ...
        _emit({"status": "STOP_METADATA_PACKAGE_ROUTE_PROBE_INTERNAL_FAIL_CLOSED", "reason": type(exc).__name__, "source_get_count": 0})
        return 13
    ...
    if observation.result_kind == "REDIRECT_STOP_REQUIRES_NEW_AUTHORIZATION":
        payload["status"] = "STOP_METADATA_PACKAGE_ROUTE_REDIRECT_REQUIRES_NEW_AUTHORIZATION"
        _emit(payload)
        return 13
    if not observation.zip_magic_present:
        payload["status"] = "STOP_METADATA_PACKAGE_ROUTE_ZIP_MAGIC_NOT_OBSERVED"
        _emit(payload)
        return 13
    payload["status"] = "PASS_METADATA_PACKAGE_ROUTE_PROBE_DIRECT_BOUNDED"
    _emit(payload)
    return 0
