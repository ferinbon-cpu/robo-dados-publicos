...
from robo_dados_publicos.productization.safe_local_mixed_execution import (  # noqa: E402
    Task213ExecutionStop,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Executa a camada contextual V4 offline, incluindo as oito receitas "
            "mistas locais aprovadas pela TASK212."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.task213_status:
        if args.ask:
            print("STOP_TASK213:ASK_AND_STATUS_ARE_MUTUALLY_EXCLUSIVE", file=sys.stderr)
            return 2
        contract = load_contract()
        payload = {
            "validation": validate_contract(),
            "promoted_questions": contract["promoted_questions"],
            "metadata_blocked_questions": contract["metadata_blocked_questions"],
    ...
    if not args.ask:
        print("STOP_TASK213:ASK_REQUIRED", file=sys.stderr)
        return 2

    generated_at, software_version = _runtime_values()
    try:
        answer = execute_contextual_query_v4(
            args.ask,
        ...
        if args.json:
            ...
            sys.stdout.write(render_contextual_answer_v4(answer)["markdown"])
    except Task213ExecutionStop as exc:
        print(f"STOP_TASK213:{exc}", file=sys.stderr)
        return 2
