...
def main() -> int:
    ...
    if args.dry_run:
        _emit(_safety("PASS_M7_SIOPE_ARTIFACT_DOWNLOAD_BLOCKED_SHAPE_DIAGNOSTICS_DRY_RUN"), args.output)
        return 0
    try:
        result = diagnose_blocked_shape(config)
    except SiopeRuntimeRouteProbeError as exc:
        _emit(_safety("STOP_M7_SIOPE_ARTIFACT_DOWNLOAD_BLOCKED_SHAPE_DIAGNOSTICS_GATE", str(exc)), args.output)
        return 30
