    ...
    try:
        if not _identity_ok():
            raise SiopeRuntimeRouteProbeError(
                "STOP_SIOPE_RUNTIME_ROUTE_ALL_TYPES_DIAGNOSTICS_RELEASE_IDENTITY"
            )
        ...
        if dry_run:
            return {
                "status": "PASS_M7_SIOPE_EXPORT_RUNTIME_ROUTE_ALL_TYPES_DIAGNOSTICS_DRY_RUN",
        ...
        return diagnose_export_runtime_route_all_types(config, runtime=runtime), 0
    except SiopeRuntimeRouteProbeError as exc:
        return {
            "status": "STOP_M7_SIOPE_EXPORT_RUNTIME_ROUTE_ALL_TYPES_DIAGNOSTICS_GATE",
            "reason": str(exc),
        ...
        return {
            "status": "STOP_M7_SIOPE_EXPORT_RUNTIME_ROUTE_ALL_TYPES_DIAGNOSTICS_UNEXPECTED",
            **_safety_fields(),
        }, 26
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="config/source_expansion.siope_export_runtime_route_probe_gate.json",
    )
