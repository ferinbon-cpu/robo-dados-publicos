...
from robo_dados_publicos.sources.siope_official_olinda_api_application_loaded_script_signature_diagnostics_review import load_json as load_review_json, run_review
from robo_dados_publicos.sources.siope_official_olinda_api_application_loaded_script_global_relation_diagnostics_design import load_json as load_design_json, run_design
...
CONFIG = ROOT / "config/source_expansion.siope_official_olinda_api_application_loaded_script_global_relation_diagnostics.json"
...
def _design_result(config: dict) -> dict:
    ...
    review_config = load_review_json(ROOT / design_config["loaded_script_review_config_path"])
...
def run_gate(*, dry: bool = False) -> dict:
    ...
    return dry_run(config, design) if dry else run_global_relation_diagnostics(config, design)
...
def main() -> int:
    ...
    try:
        ...
        exit_code = 0
    except SiopeOfficialOlindaApiApplicationLoadedScriptGlobalRelationDiagnosticsError as exc:
        result = {
            "status": "STOP_M7_SIOPE_OFFICIAL_OLINDA_API_APPLICATION_LOADED_SCRIPT_GLOBAL_RELATION_DIAGNOSTICS",
            "error_code": str(exc),
            **exc.diagnostics,
