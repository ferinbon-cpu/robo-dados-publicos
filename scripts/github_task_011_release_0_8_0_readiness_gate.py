...
def validate(data):
    ...
    if data.get("blockers") != CURRENT:
        raise ValueError("blocker matrix or protocol mapping drift")
    ...
    unmet = [key for key in CURRENT if data["blockers"].get(key) != required[key]]
    if not unmet:
        raise ValueError("TASK 011 readiness must not pass")
    return {"decision": DECISION, "unmet": unmet, "release": "0.8.0 CANDIDATE"}
