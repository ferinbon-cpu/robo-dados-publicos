...
WORKFLOW = ROOT / ".github" / "workflows" / "siope-export-request-refinement-gate.yml"
PRODUCTION = ROOT / ".github" / "workflows" / "robo-dados-publicos.yml"


class TestM7SiopeExportRequestRefinementWorkflow(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_manual_only_and_confirmation_required(self):
        ...
        self.assertIn("confirm_export_request_refinement:", self.text)
        self.assertIn("inputs.confirm_export_request_refinement == true", self.text)
    ...
    def test_full_qa_precedes_live_network(self):
        live = self.text.index("Refinamento passivo ao vivo")
    ...
    def test_no_browser_click_download_drive_or_candidate_route_commands(self):
    ...
    def test_production_workflow_cannot_reach_refinement_gate(self):
        self.assertNotIn("github_siope_export_request_refinement_gate.py", self.production)
        self.assertNotIn("M7 SIOPE EXPORT REQUEST REFINEMENT GATE", self.production)
