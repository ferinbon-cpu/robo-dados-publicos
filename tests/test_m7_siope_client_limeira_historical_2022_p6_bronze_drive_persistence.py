...
from robo_dados_publicos.sources.siope_client_limeira_historical_2022_p6_bronze_drive_persistence import (
    Historical2022P6DrivePersistenceError,
    load_json as load_drive_json,
    persist_bundle,
    validate_config,
)
from robo_dados_publicos.sources.siope_client_limeira_historical_2022_p6_bronze_single_record_capture_review import (
    Historical2022P6BronzeCaptureReviewError,
    load_json as load_review_json,
    run_review,
)

ROOT = Path(__file__).resolve().parents[1]
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2022_p6_bronze_single_record_capture_review.json"
DRIVE_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2022_p6_bronze_drive_persistence.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-historical-2022-p6-bronze-drive-persistence-gate.yml"
...
class Historical2022P6BronzeDrivePersistenceTests(unittest.TestCase):
    def test_pinned_capture_review_passes_offline(self):
        result = run_review(load_review_json(REVIEW_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_SINGLE_RECORD_CAPTURE_REVIEW")
        self.assertEqual(result["pinned_year"], 2022)
    ...
    def test_review_rejects_config_drift(self):
        ...
        with self.assertRaisesRegex(Historical2022P6BronzeCaptureReviewError, "CONFIG"):
    ...
    def test_review_rejects_pinned_evidence_blob_drift(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaisesRegex(Historical2022P6BronzeCaptureReviewError, "EVIDENCE_BLOB"):
    ...
    def test_drive_design_is_exact_create_only_and_source_offline(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_DRIVE_PERSISTENCE_DESIGN")
        self.assertFalse(result["network_called"])
        self.assertFalse(result["source_network_called"])
        self.assertEqual(result["drive_write_count"], 0)
        self.assertEqual(result["bundle_bytes"], 2081)
        self.assertEqual(result["bundle_sha256"], "68b659026fe5af968864d24fba10a6883058db4a9aa700d7f65a5a09c47ab54f")
        self.assertFalse(result["historical_collection_authorized"])

    def test_drive_config_drift_cannot_enable_unsafe_flags(self):
        base = load_drive_json(DRIVE_CONFIG)
        for key in (
            "historical_collection_authorized", "overwrite_authorized", "delete_authorized",
            "replace_authorized", "source_network_authorized", "silver_authorized", "gold_authorized",
            "processing_authorized", "recurrence_authorized", "schedule_enabled",
        ):
            config = copy.deepcopy(base)
            config[key] = True
            with self.subTest(key=key), self.assertRaisesRegex(Historical2022P6DrivePersistenceError, "CONFIG"):
    ...
    def test_mocked_drive_persists_exactly_one_historical_bundle(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_BRONZE_DRIVE_PERSISTENCE")
    ...
    def test_remote_name_collision_fails_closed_without_write(self):
        ...
        with self.assertRaisesRegex(Historical2022P6DrivePersistenceError, "REMOTE_NAME_COLLISION"):
    ...
    def test_gate_scripts_run_offline_without_drive_write(self):
        commands = [
            [sys.executable, "scripts/github_siope_client_limeira_historical_2022_p6_bronze_single_record_capture_review_gate.py"],
            [sys.executable, "scripts/github_siope_client_limeira_historical_2022_p6_bronze_drive_persistence_gate.py", "--dry-run"],
        ]
    ...
    def test_workflow_is_manual_one_create_only_no_source_get_and_no_recurrence(self):
        ...
        self.assertIn("confirm_siope_client_limeira_historical_2022_p6_bronze_drive_persistence", text)
        self.assertIn("contents: read", text)
        self.assertIn("github_siope_client_limeira_historical_2022_p6_bronze_single_record_capture_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_historical_2022_p6_bronze_drive_persistence_gate.py --dry-run", text)
