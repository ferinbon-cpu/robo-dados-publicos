...
from robo_dados_publicos.sources.siope_client_limeira_silver_drive_readback_review import (
...
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_silver_drive_readback_review.json"
GOLD_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_gold_transform_preview.json"
SILVER_PERSISTENCE_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_silver_drive_persistence.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-gold-transform-preview-gate.yml"


class GoldTransformPreviewTests(unittest.TestCase):
    def test_readback_review_accepts_exact_pinned_success(self):
        result = review(load_review_json(REVIEW_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_SILVER_DRIVE_READBACK_REVIEW")
    ...
    def test_readback_review_rejects_tampered_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SilverDriveReadbackReviewError):
    ...
    def test_readback_review_config_cannot_enable_gold_processing_or_recurrence(self):
        for field, value in (
            ...
            config = load_review_json(REVIEW_CONFIG)
            config[field] = value
            with self.assertRaises(SilverDriveReadbackReviewError, msg=field):
    ...
    def test_gold_design_is_offline_write_closed_and_no_compliance_claims(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW_DESIGN")
        self.assertFalse(result["network_called"])
    ...
    def test_exact_verified_silver_builds_expected_deterministic_metrics(self):
        payload, result = build_preview(load_json(GOLD_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_PREVIEW")
        self.assertEqual(result["metric_count"], 8)
        self.assertEqual(result["metrics"], {
            "receita_realizada_sobre_previsao_atualizada_pct": "95.3966",
            "despesa_paga_sobre_dotacao_atualizada_pct": "78.4741",
            "despesa_educacao_paga_sobre_dotacao_atualizada_educacao_pct": "86.0484",
            "participacao_educacao_na_despesa_empenhada_pct": "22.6713",
            "participacao_educacao_na_despesa_liquidada_pct": "23.5650",
            "participacao_educacao_na_despesa_paga_pct": "23.5007",
            "despesa_total_paga_por_habitante": "6097.24",
            "despesa_educacao_paga_por_habitante": "1432.89",
        })
        ...
        self.assertFalse(result["gold_remote_write_authorized"])
        self.assertEqual(len(result["gold_payload_sha256"]), 64)
    ...
    def test_gold_config_drift_cannot_enable_network_write_compliance_or_recurrence(self):
        for field, value in (
            ...
            config[field] = value
            with self.assertRaises(GoldTransformPreviewError, msg=field):
                validate_config(config)

    def test_invalid_arithmetic_inputs_fail_closed(self):
        with self.assertRaises(GoldTransformPreviewError):
    ...
    def test_tampered_silver_source_record_fails_as_prerequisite(self):
        config = load_json(GOLD_CONFIG)
    ...
    def test_gate_scripts_run_directly_without_network(self):
        review_cp = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_silver_drive_readback_review_gate.py"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        dry_cp = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_gold_transform_preview_gate.py", "--dry-run"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        preview_cp = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_gold_transform_preview_gate.py"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_workflow_is_manual_offline_full_qa_and_has_no_remote_write(self):
        ...
        self.assertIn("confirm_siope_client_limeira_gold_transform_preview", text)
        self.assertIn("contents: read", text)
        self.assertNotIn("schedule:", text)
        self.assertIn("github_siope_client_limeira_silver_drive_readback_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_gold_transform_preview_gate.py --dry-run", text)
        self.assertIn("python -m unittest discover -s tests -v", text)
        self.assertIn("python main.py selftest", text)
        self.assertNotIn("GOOGLE_OAUTH", text)
