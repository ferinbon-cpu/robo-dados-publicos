...
class TestReleaseConsistency(unittest.TestCase):
    ...
    def test_current_release_metadata_match(self):
        ...
        preserved_candidate = load_json(manifest['preserved_active_candidate_manifest'])
        current_qa = load_json('QA_SOFTWARE_V01.json')
        target_config = load_json('config/reconciliation_targets.json')

        self.assertEqual('0.6.0', manifest['current_active'])
        self.assertEqual(__version__, manifest['current_candidate'])
        self.assertEqual('0.6.0', manifest['last_active_validated'])
        self.assertEqual('0.6.0', active_manifest['version'])
        ...
        self.assertEqual('0.6.0', preserved_candidate['version'])
        ...
        self.assertIn('**Software ativo:** 0.6.0 ACTIVE', readme)
    ...
    def test_candidate_release_identity_is_explicit(self):
        ...
        self.assertEqual('0.6.0', ACTIVE_VALIDATED_VERSION)
