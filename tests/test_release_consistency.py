...
class TestReleaseConsistency(unittest.TestCase):
    ...
    def test_current_release_metadata_match(self):
        ...
        preserved_candidate_manifest = load_json(manifest['preserved_candidate_manifest'])
        current_qa = load_json('QA_SOFTWARE_V01.json')
        active_qa = load_json('QA_SOFTWARE_V01_0.6.3_ACTIVE.json')
        target_config = load_json('config/reconciliation_targets.json')

        self.assertEqual('0.6.3', manifest['current_active'])
        self.assertEqual(__version__, manifest['current_candidate'])
        self.assertEqual('0.6.3', manifest['last_active_validated'])
        self.assertEqual('0.6.3', active_manifest['version'])
        ...
        self.assertEqual('0.6.3', preserved_candidate_manifest['version'])
        self.assertEqual('CANDIDATE', preserved_candidate_manifest['status'])
        self.assertEqual(__version__, current_qa['version'])
        self.assertEqual('CANDIDATE', current_qa['status'])
        self.assertEqual('0.6.3', active_qa['version'])
        self.assertEqual('ACTIVE', active_qa['status'])
        self.assertEqual(__version__, target_config['generated_for_software'])
        readme = (root / 'README.md').read_text(encoding='utf-8')
        self.assertIn('**Software ativo:** 0.6.3 ACTIVE', readme)
    ...
    def test_candidate_release_identity_is_explicit(self):
        ...
        self.assertEqual('0.6.3', ACTIVE_VALIDATED_VERSION)
