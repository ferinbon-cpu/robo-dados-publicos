...
class TestReleaseConsistency(unittest.TestCase):
    ...
    def test_current_release_metadata_match(self):
        ...
        preserved_candidate = load_json(manifest["preserved_candidate_manifest"])
        current_qa = load_json("QA_SOFTWARE_V01.json")
        candidate_qa = load_json("QA_SOFTWARE_V01_0.8.0.json")
        active_qa = load_json("QA_SOFTWARE_V01_0.7.0_ACTIVE.json")

        self.assertEqual("0.7.0", manifest["current_active"])
        self.assertEqual(__version__, manifest["current_candidate"])
        self.assertEqual("0.7.0", manifest["last_active_validated"])
        self.assertEqual("0.7.0", active_manifest["version"])
        ...
        self.assertEqual("0.7.0", preserved_candidate["version"])
        self.assertEqual("CANDIDATE", preserved_candidate["status"])
        self.assertEqual(__version__, current_qa["version"])
        self.assertEqual("CANDIDATE", current_qa["status"])
        self.assertEqual(__version__, candidate_qa["version"])
        self.assertEqual("CANDIDATE", candidate_qa["status"])
        self.assertEqual("0.7.0", active_qa["version"])
        self.assertEqual("ACTIVE", active_qa["status"])

        readme = (root / "README.md").read_text(encoding="utf-8")
        self.assertIn("**Software ativo:** 0.7.0 ACTIVE", readme)
    ...
    def test_candidate_release_identity_is_explicit(self):
        self.assertEqual("CANDIDATE", RELEASE_STATUS)
        self.assertEqual("0.7.0", ACTIVE_VALIDATED_VERSION)
