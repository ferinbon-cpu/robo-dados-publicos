...
class TestReleaseConsistency(unittest.TestCase):
    ...
    def test_current_release_metadata_match(self):
        ...
        self.assertEqual('0.5.8', manifest['current_active'])
        self.assertEqual(__version__, manifest['current_candidate'])
        self.assertEqual('0.5.8', manifest['last_active_validated'])
        self.assertEqual('0.5.8', active_manifest['version'])
        ...
        self.assertIn(f'**Candidata:** {__version__} CANDIDATE', (root / 'README.md').read_text(encoding='utf-8'))
    ...
    def test_candidate_release_identity_is_explicit(self):
        ...
        self.assertEqual('0.5.8', ACTIVE_VALIDATED_VERSION)
