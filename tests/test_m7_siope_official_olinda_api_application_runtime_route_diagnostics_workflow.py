...
WORKFLOW = ROOT / ".github" / "workflows" / "siope-official-olinda-api-application-runtime-route-diagnostics-gate.yml"


class TestM7SiopeOfficialOlindaApiApplicationRuntimeRouteDiagnosticsWorkflow(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.text = WORKFLOW.read_text(encoding="utf-8")

    def test_manual_read_only_and_requires_exact_confirmation(self):
        self.assertIn("workflow_dispatch:", self.text)
        self.assertIn("confirm_official_olinda_application_route_diagnostics:", self.text)
        self.assertIn("contents: read", self.text)
        self.assertNotIn("schedule:", self.text)
        self.assertNotIn("push:", self.text)

    def test_design_dry_run_and_full_qa_precede_live(self):
        design = self.text.index("github_siope_official_olinda_api_application_runtime_route_diagnostics_design_gate.py")
        dry = self.text.index("github_siope_official_olinda_api_application_runtime_route_diagnostics_gate.py --dry-run")
        tests = self.text.index("python -m unittest discover -s tests -v")
        regressions = self.text.index("python main.py selftest")
        live = self.text.index("--output siope-official-olinda-application-route-diagnostics-evidence/result.json")
    ...
    def test_only_sanitized_result_is_uploaded_and_stop_propagates(self):
        self.assertIn("siope-official-olinda-application-route-diagnostics-evidence/result.json", self.text)
        self.assertIn("if: ${{ steps.live.outcome == 'failure' }}", self.text)
        self.assertIn("run: exit 1", self.text)
        self.assertNotIn("upload-artifact@v", self.text)
