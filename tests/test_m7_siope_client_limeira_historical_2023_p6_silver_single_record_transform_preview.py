...
from robo_dados_publicos.sources.siope_client_limeira_historical_2023_p6_bronze_drive_readback_review import (
    Historical2023P6BronzeDriveReadbackReviewError,
    review,
)
from robo_dados_publicos.sources.siope_client_limeira_historical_2023_p6_silver_single_record_transform_preview import (
    Historical2023P6SilverTransformPreviewError,
    preview,
    validate_config,
)

ROOT = Path(__file__).resolve().parents[1]
READBACK_REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2023_p6_bronze_drive_readback_review.json"
PREVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2023_p6_silver_single_record_transform_preview.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-historical-2023-p6-silver-single-record-transform-preview-gate.yml"
EXPECTED_SILVER_SHA256 = "6d5c6a96f7a0b57b06ad6a6b7078a46ba58ef5fd1242f08c3de8c7aa5f2c87fb"
EXPECTED_SILVER_BYTES = 1830


class SiopeHistorical2023P6SilverSingleRecordTransformPreviewTests(unittest.TestCase):
    ...
    def test_pinned_historical_readback_review_passes_offline(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_BRONZE_DRIVE_READBACK_REVIEW")
        self.assertEqual(result["pinned_run_id"], 33030848596)
    ...
    def test_readback_review_rejects_config_drift(self):
        ...
        with self.assertRaises(Historical2023P6BronzeDriveReadbackReviewError):
    ...
    def test_preview_design_is_offline_write_closed_and_single_record_only(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_SILVER_SINGLE_RECORD_TRANSFORM_PREVIEW_DESIGN")
        self.assertFalse(result["network_called"])
    ...
    def test_exact_historical_bronze_builds_expected_lossless_silver_preview(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_SILVER_SINGLE_RECORD_TRANSFORM_PREVIEW")
    ...
    def test_preview_result_does_not_expose_record_values(self):
        ...
        self.assertNotIn("1782064377.67", rendered)
    ...
    def test_config_drift_cannot_repoint_payloads_or_enable_operations(self):
        ...
        for field, value in cases:
            ...
            config[field] = value
            with self.assertRaises(Historical2023P6SilverTransformPreviewError, msg=field):
                validate_config(config)

    def test_identity_and_hash_drift_fail_closed(self):
        for mutation in (
            lambda c: c["expected_identity"].update(year=2024),
            ...
            mutation(config)
            with self.assertRaises(Historical2023P6SilverTransformPreviewError):
                validate_config(config)

    def test_gate_scripts_run_directly_from_repo_root(self):
        review_proc = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_historical_2023_p6_bronze_drive_readback_review_gate.py"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            check=False,
        ...
        preview_proc = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_historical_2023_p6_silver_single_record_transform_preview_gate.py"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            check=False,
    ...
    def test_workflow_is_manual_offline_full_qa_and_no_remote_writes(self):
        ...
        self.assertIn("confirm_siope_client_limeira_historical_2023_p6_silver_single_record_transform_preview", text)
        self.assertIn("permissions:\n  contents: read", text)
        self.assertIn("python -m unittest discover -s tests -v", text)
        self.assertIn("python main.py selftest", text)
        self.assertIn("github_siope_client_limeira_historical_2023_p6_bronze_drive_readback_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_historical_2023_p6_silver_single_record_transform_preview_gate.py --dry-run", text)
