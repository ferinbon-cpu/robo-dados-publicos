...
from robo_dados_publicos.sources.siope_client_limeira_historical_2022_p6_gold_drive_persistence_review import (
    HistoricalGoldDrivePersistenceReviewError,
    load_json as load_review_json,
    review,
)
from robo_dados_publicos.sources.siope_client_limeira_historical_2022_p6_gold_drive_readback_verification import (
...
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2022_p6_gold_drive_persistence_review.json"
READBACK_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2022_p6_gold_drive_readback_verification.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-historical-2022-p6-gold-drive-readback-verification-gate.yml"
...
class HistoricalGoldDriveReadbackVerificationTests(unittest.TestCase):
    def test_persistence_review_passes_exact_pinned_evidence(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_GOLD_DRIVE_PERSISTENCE_REVIEW")
        self.assertTrue(result["drive_readback_design_authorized"])
        self.assertEqual(result["pinned_run_id"], 33099147098)
    ...
    def test_readback_design_is_network_free_write_closed_and_historical_collection_closed(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_GOLD_DRIVE_READBACK_VERIFICATION_DESIGN")
        self.assertFalse(result["network_called"])
    ...
    def test_expected_payload_is_exact_historical_gold(self):
        ...
        self.assertEqual(hashlib.sha256(expected_bytes).hexdigest(), "4057aac2b18dc7184db992ee989d64c8732c4ad858cc6e8b7520cd50c4d37f68")
    ...
    def test_mocked_live_readback_verifies_exact_bytes_without_writes(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2022_P6_GOLD_DRIVE_READBACK_VERIFICATION")
    ...
    def test_gate_scripts_run_directly_in_offline_mode(self):
        review_cp = subprocess.run([sys.executable, "scripts/github_siope_client_limeira_historical_2022_p6_gold_drive_persistence_review_gate.py"], cwd=ROOT, text=True, capture_output=True, check=False)
        self.assertEqual(review_cp.returncode, 0, review_cp.stderr or review_cp.stdout)
        readback_cp = subprocess.run([sys.executable, "scripts/github_siope_client_limeira_historical_2022_p6_gold_drive_readback_verification_gate.py", "--dry-run"], cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    def test_workflow_is_manual_readonly_full_qa_and_no_drive_write(self):
        ...
        self.assertIn("confirm_siope_client_limeira_historical_2022_p6_gold_drive_readback_verification", text)
        self.assertIn("contents: read", text)
        self.assertNotIn("schedule:", text)
        self.assertIn("python -m unittest discover -s tests -v", text)
        self.assertIn("python main.py selftest", text)
        self.assertIn("github_siope_client_limeira_historical_2022_p6_gold_drive_persistence_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_historical_2022_p6_gold_drive_readback_verification_gate.py --dry-run", text)
        self.assertNotIn("github_siope_client_limeira_historical_2022_p6_gold_drive_persistence_gate.py >", text)
