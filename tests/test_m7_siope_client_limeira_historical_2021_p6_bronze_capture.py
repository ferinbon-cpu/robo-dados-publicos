...
from robo_dados_publicos.sources.siope_client_limeira_historical_2021_p6_bronze_single_record_capture import (
    Historical2021P6BronzeCaptureError,
    capture,
    validate_config,
)
from robo_dados_publicos.sources.siope_client_limeira_historical_2021_p6_full_schema_readonly_validation_review import (
...
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2021_p6_full_schema_readonly_validation_review.json"
CAPTURE_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2021_p6_bronze_single_record_capture.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-historical-2021-p6-bronze-single-record-capture-gate.yml"
...
class Historical2021P6BronzeCaptureTests(unittest.TestCase):
    def test_pinned_historical_validation_review_passes_offline(self):
        result = review(load_review_json(REVIEW_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2021_P6_FULL_SCHEMA_READONLY_VALIDATION_REVIEW")
        self.assertEqual(result["pinned_year"], 2021)
    ...
    def test_review_rejects_config_drift(self):
        ...
        with self.assertRaisesRegex(Historical2021P6ValidationReviewError, "CONFIG_DRIFT"):
    ...
    def test_review_rejects_evidence_blob_drift(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaisesRegex(Historical2021P6ValidationReviewError, "EVIDENCE_BLOB_DRIFT"):
                review(config, root=root)
    ...
    def test_capture_config_drift_cannot_enable_bulk_drive_or_recurrence(self):
        ...
        for key in ("historical_collection_authorized", "drive_persistence_authorized", "processing_authorized", "recurrence_authorized", "schedule_enabled"):
            ...
            config[key] = True
            with self.subTest(key=key), self.assertRaises(Historical2021P6BronzeCaptureError):
    ...
    def test_mocked_capture_persists_exact_public_record_and_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2021_P6_BRONZE_SINGLE_RECORD_CAPTURE")
            self.assertEqual(result["request_count"], 1)
            self.assertEqual(result["selected_schema_key_count"], 52)
            self.assertEqual(persisted, record)
            self.assertEqual(manifest["year"], 2021)
            self.assertEqual(manifest["period"], 6)
            self.assertEqual(manifest["validation_prerequisite_run_id"], 33105190675)
    ...
    def test_capture_schema_drift_fails_closed(self):
        ...
        record = full_record_2021()
        record.pop("VAL_PIB")
        with tempfile.TemporaryDirectory() as td, self.assertRaisesRegex(Historical2021P6BronzeCaptureError, "SCHEMA"):
    ...
    def test_capture_identity_drift_fails_closed(self):
        ...
        for key, wrong in (("COD_MUNI", 1), ("NUM_ANO", 2022), ("NUM_PERI", 5), ("SIG_UF", "RJ")):
            record = full_record_2021()
            record[key] = wrong
            with tempfile.TemporaryDirectory() as td, self.subTest(key=key), self.assertRaises(Historical2021P6BronzeCaptureError):
                capture(config, output_dir=td, opener=opener_for({"value": [record]}))

    def test_gate_scripts_run_directly_without_live_network(self):
        commands = [
            [sys.executable, "scripts/github_siope_client_limeira_historical_2021_p6_full_schema_readonly_validation_review_gate.py"],
            [sys.executable, "scripts/github_siope_client_limeira_historical_2021_p6_bronze_single_record_capture_gate.py", "--dry-run"],
        ]
    ...
    def test_workflow_is_manual_one_get_local_capture_and_no_drive(self):
        ...
        self.assertIn("confirm_siope_client_limeira_historical_2021_p6_bronze_single_record_capture", text)
        self.assertIn("contents: read", text)
        self.assertIn("github_siope_client_limeira_historical_2021_p6_full_schema_readonly_validation_review_gate.py", text)
    ...
    def test_workflow_orders_review_and_full_qa_before_capture(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        review_at = text.index("github_siope_client_limeira_historical_2021_p6_full_schema_readonly_validation_review_gate.py")
        tests_at = text.index("python -m unittest discover -s tests -v")
        selftest_at = text.index("python main.py selftest")
        live_at = text.index("Capturar exatamente um registro Bronze de Limeira 2021 P6")
