...
class TestTask202EquityMissingnessAwareGate(unittest.TestCase):
    def _products_after(self):
        products = full_products(with_task201b=True)
        products["TERRITORY_PROFILE"] = build_task202_territory_profile(
            generated_at=GENERATED_AT,
            software_version=SOFTWARE_VERSION,
        )
        return products

    def test_contract_pins_59_linked_10_held_and_no_full_network(self):
        report = validate_contract()
        self.assertEqual(report["status"], "PASS")
        self.assertEqual(report["strong_links"], 59)
        self.assertEqual(report["held"], 10)
        self.assertAlmostEqual(report["coverage_rate"], 59 / 69)
        self.assertEqual(report["unresolved_promoted"], 0)
        self.assertFalse(report["full_network"])
        coverage = validated_coverage()
        self.assertEqual(len(coverage["linked_codes"]), 59)
        self.assertEqual(len(coverage["held_codes"]), 10)
    ...
    def test_territory_product_adds_only_narrow_missingness_capability(self):
        ...
        self.assertEqual(len(school_codes), 59)

    def test_v4_changes_only_equity_q1_and_reaches_38_of_38(self):
        before_products = full_products(with_task201b=True)
        after_products = self._products_after()
        before = question_answerability(before_products, answerability_path=ANSWERABILITY_V3)
        after = current_question_answerability_v4(after_products)
        self.assertEqual(
    ...
    def test_v4_preserves_equity_metrics_and_removes_only_full_network_gate(self):
        ...
        self.assertNotIn(
            "SCHOOL_TO_SECTOR_LINK_FULL_NETWORK",
            territory["required_capabilities"],
        )
    ...
    def test_contract_fails_closed_on_coverage_or_held_roster_drift(self):
        original = json.loads(Path(ROOT / "config/task202_equity_missingness_aware_gate.v1.json").read_text(encoding="utf-8"))
        mutations = []
        low_links = copy.deepcopy(original)
        low_links["coverage"]["strong_links"] = 58
        mutations.append(low_links)
        low_rate = copy.deepcopy(original)
        low_rate["coverage"]["rate"] = 0.84
    ...
    def test_v3_remains_37_of_38_with_task202_product(self):
        products = self._products_after()
        report = question_answerability(products, answerability_path=ANSWERABILITY_V3)
        self.assertEqual(
