...
from robo_dados_publicos.sources.siope_client_limeira_silver_drive_persistence_review import (
    SilverDrivePersistenceReviewError,
    load_json as load_review_json,
    review,
)
from robo_dados_publicos.sources.siope_client_limeira_silver_drive_readback_verification import (
...
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_silver_drive_persistence_review.json"
READBACK_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_silver_drive_readback_verification.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-silver-drive-readback-verification-gate.yml"
...
class SilverDriveReadbackVerificationTests(unittest.TestCase):
    def test_persistence_review_passes_exact_pinned_evidence(self):
        result = review(load_review_json(REVIEW_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_SILVER_DRIVE_PERSISTENCE_REVIEW")
    ...
    def test_tampered_persistence_evidence_fails_closed(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SilverDrivePersistenceReviewError):
    ...
    def test_readback_design_is_network_free_write_closed_and_gold_closed(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_SILVER_DRIVE_READBACK_VERIFICATION_DESIGN")
        self.assertFalse(result["network_called"])
    ...
    def test_mocked_live_readback_verifies_exact_bytes_without_writes(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_SILVER_DRIVE_READBACK_VERIFICATION")
        ...
        self.assertFalse(result["gold_authorized"])
        self.assertEqual(result["silver_contract"], "SIOPE_DADOS_GERAIS_LIMEIRA_VALIDATED_RECORD_SILVER_V1")
    ...
    def test_tampered_remote_payload_fails_closed(self):
        ...
        with self.assertRaises(SilverDriveReadbackVerificationError):
    ...
    def test_config_drift_cannot_enable_writes_source_gold_processing_or_recurrence(self):
        for field, value in (
            ...
            config = load_json(READBACK_CONFIG)
            config[field] = value
            with self.assertRaises(SilverDriveReadbackVerificationError, msg=field):
                validate_config(config, root=ROOT)

    def test_gate_scripts_run_directly_in_offline_mode(self):
        review_cp = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_silver_drive_persistence_review_gate.py"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        readback_cp = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_silver_drive_readback_verification_gate.py", "--dry-run"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_workflow_is_manual_readonly_full_qa_and_no_drive_write(self):
        ...
        self.assertIn("confirm_siope_client_limeira_silver_drive_readback_verification", text)
        self.assertIn("contents: read", text)
        self.assertNotIn("schedule:", text)
        self.assertIn("python -m unittest discover -s tests -v", text)
        self.assertIn("python main.py selftest", text)
        self.assertIn("github_siope_client_limeira_silver_drive_persistence_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_silver_drive_readback_verification_gate.py --dry-run", text)
        self.assertNotIn("github_siope_client_limeira_silver_drive_persistence_gate.py >", text)
