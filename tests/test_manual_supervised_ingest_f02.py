...
def contract(family: str) -> F02SourceContract:
...
@pytest.mark.parametrize(
    ("text", "family"),
    [(RREO_TEXT, "RREO_MDE"), (FUNDEB_TEXT, "FUNDEB_LOCAL"), (MDE25_TEXT, "MDE_25_LOCAL")],
)
def test_classifier_exactly_one_family(text, family):
    assert classify_f02_text(text) == family


def test_classifier_unknown_is_stop():
    with pytest.raises(F02IngestStop, match="UNKNOWN_DOCUMENT"):
        classify_f02_text("documento sem assinatura conhecida")


def test_classifier_ambiguous_is_stop():
    with pytest.raises(F02IngestStop, match="AMBIGUOUS_DOCUMENT"):
        classify_f02_text(FUNDEB_TEXT + "\n" + MDE25_TEXT)


def test_rreo_parser_pins_official_values():
    record = parse_rreo_mde(RREO_TEXT)
    assert record["period_end"] == "2026-04-30"
    assert record["authority"] == "OFFICIAL_MDE_PRIMARY"
    assert record["metrics"]["tax_revenue_realized"] == "477974870.08"
    assert record["metrics"]["fundeb_limit_expense_liquidated"] == "62841461.94"
    assert record["metrics"]["fundeb_professionals_percent"] == "88.67"
    assert record["metrics"]["mde_percent"] == "24.27"


def test_local_parsers_preserve_methodology_specific_percentages():
    fundeb = parse_fundeb_local(FUNDEB_TEXT)
    mde25 = parse_mde_25_local(MDE25_TEXT)
    assert fundeb["period_end"] == mde25["period_end"] == "2026-04-30"
    assert fundeb["metrics"]["fundeb_application_liquidated_percent"] == "88.93"
    assert fundeb["metrics"]["fundeb_professionals_liquidated_percent_local"] == "88.62"
    assert mde25["metrics"]["education_expense_liquidated_percent"] == "23.23"
    assert mde25["metrics"]["education_net_expense_liquidated_percent"] == "23.19"


def test_normalizer_rejects_contract_family_mismatch():
    with pytest.raises(F02IngestStop, match="CONTRACT_MISMATCH"):
        normalize_f02_document(contract("RREO_MDE"), FUNDEB_TEXT)


def test_reconciliation_preserves_exact_matches_and_methodology_differences():
    records = [
        parse_rreo_mde(RREO_TEXT),
        parse_fundeb_local(FUNDEB_TEXT),
        parse_mde_25_local(MDE25_TEXT),
    ]
    result = reconcile_f02(records)
    assert result["status"] == "PASS_F02_RECONCILIATION"
    assert all(result["exact_checks"].values())
    assert result["methodology_differences"]["fundeb_retained_local_minus_rreo_destined"] == "-0.20"
    assert result["methodology_differences"]["mde_liquidated_percent_local_minus_rreo_official"] == "-1.04"
    assert result["authority_rule"] == "RREO_MDE_FOR_OFFICIAL_MDE_CLAIMS"
    assert result["promotion"]["gold_authorized"] is False


def test_reconciliation_period_mismatch_is_stop():
    records = [
        parse_rreo_mde(RREO_TEXT),
        parse_fundeb_local(FUNDEB_TEXT),
        parse_mde_25_local(MDE25_TEXT),
    ...
    with pytest.raises(F02IngestStop, match="PERIOD_MISMATCH"):
        reconcile_f02(records)


def test_reconciliation_expected_exact_mismatch_is_stop():
    records = [
        parse_rreo_mde(RREO_TEXT),
        parse_fundeb_local(FUNDEB_TEXT),
        parse_mde_25_local(MDE25_TEXT),
    ...
    with pytest.raises(F02IngestStop, match="EXPECTED_EXACT_MISMATCH"):
        reconcile_f02(records)


def test_immutable_validator_checks_hash_bytes_pages_and_text(monkeypatch):
    payload = b"abc"
    import hashlib
    ...
    monkeypatch.setattr(
        ...
        lambda _: {"pages": 7, "text_pages": 7, "text_chars": 10, "has_text_layer": True, "text": RREO_TEXT},
    )
    result = validate_f02_source_bytes(pinned, payload)
    assert result["status"] == "PASS_F02_SOURCE_BYTES_VERIFIED"
    ...
    with pytest.raises(F02IngestStop, match="IMMUTABLE_MISMATCH"):
        validate_f02_source_bytes(bad, payload)


def test_plan_forbids_promotion(tmp_path):
    config = {
        ...
        "sources": [
            ...
            for i, family in enumerate(("RREO_MDE", "FUNDEB_LOCAL", "MDE_25_LOCAL"), start=1)
        ],
        "promotion": {
    ...
    path = tmp_path / "f02.json"
    path.write_text(json.dumps(config), encoding="utf-8")
    assert len(load_f02_ingest_plan(str(path))["contracts"]) == 3

    config["promotion"]["silver_authorized_by_this_contract"] = True
    path.write_text(json.dumps(config), encoding="utf-8")
    with pytest.raises(F02IngestStop, match="UNAUTHORIZED_PROMOTION"):
