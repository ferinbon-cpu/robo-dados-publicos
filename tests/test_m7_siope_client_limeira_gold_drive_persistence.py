...
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_gold_transform_review.json"
PERSISTENCE_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_gold_drive_persistence.json"
CLOUD_CONFIG = ROOT / "config/cloud.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-gold-drive-persistence-gate.yml"
...
class GoldDrivePersistenceTests(unittest.TestCase):
    def test_pinned_gold_preview_review_passes_offline(self):
        result = review(load_review_json(REVIEW_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_REVIEW")
        ...
        self.assertEqual(result["gold_payload_bytes"], 1612)
        self.assertEqual(
            result["gold_payload_sha256"],
            "d6a35db7c42129569c73f19de789d871d0d285929d8eb3fe2a04d5ef03fdd6e0",
        )
    ...
    def test_review_rejects_tampered_pinned_identity(self):
        for key, value in (
            ...
            with self.subTest(key=key):
                ...
                config[key] = value
                with self.assertRaises(GoldTransformReviewError):
    ...
    def test_persistence_design_rebuilds_exact_gold_without_network(self):
        result = validate_persistence(load_persistence_json(PERSISTENCE_CONFIG), root=ROOT)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_DRIVE_PERSISTENCE_DESIGN")
        self.assertFalse(result["network_called"])
    ...
    def test_mocked_live_creates_exactly_one_validated_gold(self):
        drive = FakeDrive()
        result = persist(load_persistence_json(PERSISTENCE_CONFIG), root=ROOT, drive=drive)
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_DRIVE_PERSISTENCE")
        ...
        self.assertTrue(remote_name.endswith("__d6a35db7c421__gold_v1.json"))
        self.assertEqual(len(raw), 1612)
        self.assertEqual(hashlib.sha256(raw).hexdigest(), result["gold_payload_sha256"])
        payload = json.loads(raw.decode("utf-8"))
        self.assertEqual(payload["gold_contract"], "SIOPE_DADOS_GERAIS_LIMEIRA_ARITHMETIC_SUMMARY_GOLD_V1")
    ...
    def test_remote_name_collision_stops_before_write(self):
        ...
        with self.assertRaises(GoldDrivePersistenceError):
    ...
    def test_gate_scripts_run_directly_without_live_network(self):
        review_run = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_gold_transform_review_gate.py"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        self.assertIn("PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_TRANSFORM_REVIEW", review_run.stdout)
        design_run = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_gold_drive_persistence_gate.py", "--dry-run"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        self.assertIn("PASS_M7_SIOPE_CLIENT_LIMEIRA_GOLD_DRIVE_PERSISTENCE_DESIGN", design_run.stdout)

    def test_workflow_is_manual_create_only_and_full_qa_precedes_live(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("workflow_dispatch:", text)
        self.assertIn("confirm_siope_client_limeira_gold_drive_persistence", text)
        self.assertIn("permissions:\n  contents: read", text)
        self.assertNotIn("schedule:", text)
        self.assertIn("persist-credentials: false", text)
        self.assertIn("GOOGLE_DRIVE_CLIENT_ID: ${{ secrets.GOOGLE_DRIVE_CLIENT_ID }}", text)
        self.assertIn("github_siope_client_limeira_gold_transform_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_gold_drive_persistence_gate.py --dry-run", text)
        unit_pos = text.index("python -m unittest discover -s tests -v")
        regression_pos = text.index("python main.py selftest")
        live_pos = text.index("Persistir exatamente um payload Gold validado no 03_GOLD")
