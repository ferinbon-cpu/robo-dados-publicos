...
class TestSiope2025OfficialAliasDocumentaryDiscoveryReview(unittest.TestCase):
    def test_current_review_passes(self) -> None:
        review, evidence, auth = _base()
        self.assertEqual(module.validate(review, evidence, auth), module.PASS)
    ...
    def test_fail_closed_mutations(self) -> None:
        ...
        for index, (mutation, target) in enumerate(mutations, start=1):
            with self.subTest(mutation=index, target=target):
                review, evidence, auth = _base()
                ...
                mutation(obj)
                with self.assertRaises(module.ReviewGateError):
                    module.validate(review, evidence, auth)

    def test_blocked_sharepoint_route_cannot_appear_in_session_attempts(self) -> None:
        review, evidence, auth = _base()
        evidence = copy.deepcopy(evidence)
