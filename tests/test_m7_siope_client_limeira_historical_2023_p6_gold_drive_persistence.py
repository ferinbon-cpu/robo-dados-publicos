...
from robo_dados_publicos.sources.siope_client_limeira_historical_2023_p6_gold_drive_persistence import (
...
REVIEW_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2023_p6_gold_transform_review.json"
PERSISTENCE_CONFIG = ROOT / "config/source_expansion.siope_client_limeira_historical_2023_p6_gold_drive_persistence.json"
CLOUD_CONFIG = ROOT / "config/cloud.json"
WORKFLOW = ROOT / ".github/workflows/siope-client-limeira-historical-2023-p6-gold-drive-persistence-gate.yml"
...
class HistoricalGoldDrivePersistenceTests(unittest.TestCase):
    def test_pinned_historical_gold_preview_review_passes_offline_and_blob(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_TRANSFORM_REVIEW")
    ...
    def test_persistence_design_rebuilds_exact_historical_gold_without_network(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE_DESIGN")
        self.assertFalse(result["network_called"])
    ...
    def test_mocked_live_creates_exactly_one_validated_historical_gold(self):
        ...
        self.assertEqual(result["status"], "PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE")
        ...
        self.assertEqual(remote_name, "FNDE_SIOPE_DADOS_INFORMADOS_MUNICIPIOS_LIMEIRA__Dados_Gerais_Siope__Limeira_SP__2023_P6__352690__a4da994fd2a0__gold_v1.json")
    ...
    def test_gate_scripts_run_directly_without_live_network(self):
        review_run = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_historical_2023_p6_gold_transform_review_gate.py"],
            cwd=ROOT, text=True, capture_output=True, check=False,
        )
        self.assertEqual(review_run.returncode, 0, review_run.stderr + review_run.stdout)
        self.assertIn("PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_TRANSFORM_REVIEW", review_run.stdout)
        design_run = subprocess.run(
            [sys.executable, "scripts/github_siope_client_limeira_historical_2023_p6_gold_drive_persistence_gate.py", "--dry-run"],
            cwd=ROOT, text=True, capture_output=True, check=False,
        )
        self.assertEqual(design_run.returncode, 0, design_run.stderr + design_run.stdout)
        self.assertIn("PASS_M7_SIOPE_CLIENT_LIMEIRA_HISTORICAL_2023_P6_GOLD_DRIVE_PERSISTENCE_DESIGN", design_run.stdout)

    def test_workflow_is_manual_create_only_and_full_qa_precedes_live(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("workflow_dispatch:", text)
        self.assertIn("confirm_siope_client_limeira_historical_2023_p6_gold_drive_persistence", text)
        self.assertIn("permissions:\n  contents: read", text)
        self.assertNotIn("schedule:", text)
        self.assertIn("persist-credentials: false", text)
        self.assertIn("GOOGLE_DRIVE_CLIENT_ID: ${{ secrets.GOOGLE_DRIVE_CLIENT_ID }}", text)
        self.assertIn("github_siope_client_limeira_historical_2023_p6_gold_transform_review_gate.py", text)
        self.assertIn("github_siope_client_limeira_historical_2023_p6_gold_drive_persistence_gate.py --dry-run", text)
        unit_pos = text.index("python -m unittest discover -s tests -v")
        regression_pos = text.index("python main.py selftest")
        live_pos = text.index("Persistir exatamente um payload Gold histórico 2023 P6 validado no 03_GOLD")
