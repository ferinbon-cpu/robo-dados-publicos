...
from robo_dados_publicos.research.task217f_jom_12_document_recovery import (
    load_config,
...
class TestTask217FJom12DocumentRecovery(unittest.TestCase):
    def test_exact_12_targets_and_budget_math(self):
        cfg = load_config()
        self.assertEqual(len(cfg["targets"]), 12)
        self.assertEqual(
            [row["edition"] for row in cfg["targets"]],
            [7243,7253,7265,7270,7271,7287,7294,7316,7317,7318,7319,7320],
        )
        self.assertEqual(cfg["network"]["max_total_remote_get_count"], 12)
        self.assertEqual(cfg["network"]["max_bytes_per_document"], 89128960)
    ...
    def test_offline_carrier_is_inert(self):
        ...
        self.assertEqual(got["target_count"], 12)
    ...
    def test_consumed_authorizations_are_rejected(self):
        ...
        self.assertEqual(
            validate_live_authorization(
                {"task018_authorization_reused": True},
    ...
    def test_exact_live_authorization_shape(self):
        ...
        auth = {
            ...
            "max_document_get_attempt_count": 12,
            "max_total_remote_get_count": 12,
            "max_bytes_per_document": 89128960,
